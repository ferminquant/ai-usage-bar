...
class PackagingContractTests(unittest.TestCase):
    ...
    def test_installer_is_user_scoped_and_transactional(self):
        ...
        self.assertIn("StartupEnabled", self.install)
        self.assertIn("Get-StartupPreference", self.install)
        self.assertIn("startup_enabled", self.install)
        self.assertIn("Restore-StartupPreference", self.install)
        self.assertIn("if (-not (Test-Path -LiteralPath $runKey))", self.install)
    ...
    def test_shell_menu_exposes_startup_toggle(self):
        ...
        self.assertIn(
            "STARTUP_PREFERENCE_VALUE_NAME",
            (ROOT / "src" / "startup.rs").read_text(encoding="utf-8"),
        )
    ...
    def test_uninstaller_requires_marker_and_does_not_remove_config(self):
        ...
        self.assertIn("StartupEnabled", self.uninstall)
    ...
    def test_smoke_covers_startup_upgrade_and_preservation(self):
        for marker in (
            ...
            "TestFailureMode",
            "preservationSentinelName",
            "Installer did not preserve unrelated Run values",
        ):
