...
| Ollama Pro/cloud | Hosted session (5-hour) and weekly (7-day) quota windows, plus plan/reset state | Primary: authenticated `GET https://ollama.com/api/usage` for fractions; optional authenticated settings fetch for reset timestamps in `data-time` attributes; `ollama launch` has no usage command | `/api/usage` is live but not in the public API docs; settings HTML and browser-session auth can change; key/cookie material must remain private | **Hybrid totals + reset enrichment** in issue #9; see [ollama-spike.md](spikes/ollama-spike.md) |
