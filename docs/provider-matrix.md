...
| OpenCode Go | USD-weighted five-hour ($12), weekly ($30), and monthly ($60) plan windows; values, model tiers, and multipliers can change | `/connect` Go API key stored by OpenCode in `~/.local/share/opencode/auth.json`; local session ledger plus published model Usage tiers can produce an explicitly inferred estimate | Console export is service-account-only historical CSV; proposed `/zen/go/v1/usage` remains in upstream issue/PR and returned 404 in a live probe; local data misses other devices/workspaces and authoritative reset metadata | **Exact adapter deferred; local estimator is a separate opt-in/inferred path** — see [opencode-go-spike.md](spikes/opencode-go-spike.md) |
