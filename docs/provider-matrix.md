...
| Ollama Pro/cloud | Hosted session (5-hour) and weekly (7-day) quota windows, plus plan/reset state | Authenticated `GET https://ollama.com/api/usage` returns session/weekly fractions; `ollama launch` has no usage command; settings page supplies reset semantics only | `/api/usage` is live but not in the public API docs and has no reset timestamps; key signing must remain private | **Totals source verified; implement adapter** — see [ollama-spike.md](spikes/ollama-spike.md) |
