...
| OpenCode Go | USD-weighted five-hour ($12), weekly ($30), and monthly ($60) plan windows; values, model tiers, and multipliers can change | Authenticated `GET https://opencode.ai/zen/go/v1/usage` using the existing `opencode-go` key in `~/.local/share/opencode/auth.json`; local `message` ledger estimate remains the no-key fallback | The individual-key usage endpoint is not documented in the public CLI reference and may change; if it is unavailable, the adapter reports the cached result as stale rather than silently presenting a local estimate | **Implemented as an exact account source with an inferred local fallback**; see [opencode-go-spike.md](spikes/opencode-go-spike.md) |
