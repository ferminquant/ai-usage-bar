...
| OpenCode Go | USD-weighted five-hour ($12), weekly ($30), and monthly ($60) plan windows; values and model mix can change | `/connect` Go API key stored by OpenCode in `~/.local/share/opencode/auth.json`; current docs expose model endpoints and console link, not a quota endpoint | Console export is service-account-only historical CSV; proposed `/zen/go/v1/usage` remains in upstream issue/PR and returned 404 in a live probe | **Spike complete; defer adapter** until an individual-key endpoint is released and documented — see [opencode-go-spike.md](spikes/opencode-go-spike.md) |
