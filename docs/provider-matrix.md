...
| Ollama Pro/cloud | Hosted session (5-hour) and weekly (7-day) quota windows, plus plan/reset state | Account settings page currently shows totals; `ollama launch` has no usage command; documented Cloud/API surface does not expose account totals | No supported non-scraping source or documented account-usage endpoint yet | **Spike recorded; defer adapter** — see [ollama-spike.md](spikes/ollama-spike.md) |
